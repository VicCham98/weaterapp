...
const Home = (props) => {
  ...
  const getWeather = async () => {
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${MyCity}&APPID=47df65cae48a95e710f4b8274d749a2a&units=metric`,
    )
      .then(data => data.json())
      .then(results => {
        setInfo({
  ...
  if (props.route.params.city != 'Peru') {
    getWeather();
  }
  return (
    <View style={{flex: 1, backgroundColor: '#e3e3e3'}}>
      ...
      <View style={{alignItems: 'center'}}>
        ...
        <Image
          ...
          source={{
            uri: 'http://openweathermap.org/img/w/' + info.icon + '.png',
          }}
        />
      </View>
